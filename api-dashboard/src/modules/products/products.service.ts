...
// Generate abbreviation from name (first letter of each word)
function generateAbbreviation(name: string): string {
  if (!name) return '';

  // Remove diacritics first, then get first letter of each word
...
export class ProductsService {
  ...
  async create(tenantId: string, brandId: string, createDto: CreateProductDto) {
    ...
    const searchName = removeVietnameseTones(createDto.name).toLowerCase();
    const abbreviation = createDto.abbreviation || generateAbbreviation(createDto.name);
  ...
  async update(tenantId: string, id: string, updateDto: UpdateProductDto) {
    ...
    // Update search fields if name changes
    if (updateDto.name) {
      updateDto['searchName'] = removeVietnameseTones(updateDto.name).toLowerCase();
      // Only auto-generate abbreviation if not explicitly provided
      if (!updateDto.abbreviation) {
        updateDto['abbreviation'] = generateAbbreviation(updateDto.name);
      }
    }
