...
export const fetchBranchesByBrand = createAsyncThunk(
  ...
  async (brandId: string, { getState, rejectWithValue }) => {
    try {
      ...
      const data = await branchService.getByBrand(brandId);
...
const branchesSlice = createSlice({
  ...
  reducers: {
    ...
    addBranch: (state, action: PayloadAction<{ brandId: string; branch: Branch }>) => {
      const { brandId, branch } = action.payload;
      if (!state.byBrandId[brandId]) {
        state.byBrandId[brandId] = [];
      }
      state.byBrandId[brandId].push(branch);
    },
  },
...
export const { clearBranches, invalidateBranchCache, addBranch } = branchesSlice.actions;
