...
export const permissionService = {
  ...
  getDepartmentPermissions: async (departmentId: string): Promise<Permission[]> => {
    const response = await api.get<Permission[]>(`/api/permissions/department/${departmentId}`);
    return response.data;
  },
  ...
  getStaffPermissions: async (staffId: string): Promise<StaffPermissionsResponse> => {
    const response = await api.get<StaffPermissionsResponse>(`/api/permissions/staff/${staffId}`);
    return response.data;
  },

  // Get staff's own permissions only
  getStaffOwnPermissions: async (staffId: string): Promise<Permission[]> => {
    const response = await api.get<Permission[]>(`/api/permissions/staff/${staffId}/own`);
    return response.data;
  },
  ...
  getStaffPermissionCodes: async (staffId: string): Promise<string[]> => {
    const response = await api.get<string[]>(`/api/permissions/staff/${staffId}/codes`);
    return response.data;
  },
};
