...
export const settingsService = {
  ...
  getAllPaymentMethods: async (brandId: string): Promise<PaymentMethod[]> => {
    const response = await api.get(`/api/settings/payment-methods?brandId=${brandId}`);
    return response.data;
  },

  getPaymentMethod: async (id: string): Promise<PaymentMethod> => {
    const response = await api.get(`/api/settings/payment-methods/${id}`);
    return response.data;
  },

  createPaymentMethod: async (brandId: string, data: CreatePaymentMethodDto): Promise<PaymentMethod> => {
    const response = await api.post(`/api/settings/payment-methods?brandId=${brandId}`, data);
    return response.data;
  },

  updatePaymentMethod: async (id: string, data: UpdatePaymentMethodDto): Promise<PaymentMethod> => {
    const response = await api.put(`/api/settings/payment-methods/${id}`, data);
    return response.data;
  },

  togglePaymentMethod: async (id: string): Promise<PaymentMethod> => {
    const response = await api.patch(`/api/settings/payment-methods/${id}/toggle`);
    return response.data;
  },
  ...
  getAllBankAccounts: async (brandId: string): Promise<BankAccount[]> => {
    const response = await api.get(`/api/settings/bank-accounts?brandId=${brandId}`);
    return response.data;
  },

  getBankAccount: async (id: string): Promise<BankAccount> => {
    const response = await api.get(`/api/settings/bank-accounts/${id}`);
    return response.data;
  },

  createBankAccount: async (brandId: string, data: CreateBankAccountDto): Promise<BankAccount> => {
    const response = await api.post(`/api/settings/bank-accounts?brandId=${brandId}`, data);
    return response.data;
  },

  updateBankAccount: async (id: string, data: UpdateBankAccountDto): Promise<BankAccount> => {
    const response = await api.put(`/api/settings/bank-accounts/${id}`, data);
    return response.data;
  },

  toggleBankAccount: async (id: string): Promise<BankAccount> => {
    const response = await api.patch(`/api/settings/bank-accounts/${id}/toggle`);
    return response.data;
  },

  deleteBankAccount: async (id: string): Promise<void> => {
    await api.delete(`/api/settings/bank-accounts/${id}`);
  },
  ...
  generateVietQR: async (id: string, amount?: number, description?: string): Promise<VietQRResponse> => {
    ...
    const response = await api.get(`/api/settings/bank-accounts/${id}/qr?${params.toString()}`);
    return response.data;
  },

  // E-Invoice Configs
  getAllEInvoiceConfigs: async (brandId: string): Promise<EInvoiceConfig[]> => {
    const response = await api.get(`/api/settings/einvoice-configs?brandId=${brandId}`);
    return response.data;
  },

  getEInvoiceConfig: async (id: string): Promise<EInvoiceConfig> => {
    const response = await api.get(`/api/settings/einvoice-configs/${id}`);
    return response.data;
  },

  createEInvoiceConfig: async (brandId: string, data: CreateEInvoiceConfigDto): Promise<EInvoiceConfig> => {
    const response = await api.post(`/api/settings/einvoice-configs?brandId=${brandId}`, data);
    return response.data;
  },

  updateEInvoiceConfig: async (id: string, data: UpdateEInvoiceConfigDto): Promise<EInvoiceConfig> => {
    const response = await api.put(`/api/settings/einvoice-configs/${id}`, data);
    return response.data;
  },

  toggleEInvoiceConfig: async (id: string): Promise<EInvoiceConfig> => {
    const response = await api.patch(`/api/settings/einvoice-configs/${id}/toggle`);
    return response.data;
  },

  deleteEInvoiceConfig: async (id: string): Promise<void> => {
    await api.delete(`/api/settings/einvoice-configs/${id}`);
  },
};
