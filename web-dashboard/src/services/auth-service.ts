...
export const authService = {
  ...
  login: async (credentials: LoginCredentials): Promise<LoginResponse> => {
    const response = await apiDashboard.post<DashboardLoginResponse>("/api/auth/login", {
  ...
  changePassword: async (currentPassword: string, newPassword: string): Promise<void> => {
    await apiDashboard.post("/api/auth/change-password", {
      currentPassword,
      newPassword,
    });
  },
  ...
  getMe: async (): Promise<LoginResponse> => {
    const response = await apiDashboard.get<{ user: DashboardLoginResponse["user"] }>("/api/auth/me");
  ...
  forgotPassword: async (tenantId: string, email: string): Promise<void> => {
    await apiDashboard.post("/api/auth/forgot-password", { tenantId, email });
  },
  ...
  logout: async (): Promise<void> => {
    try {
      await apiDashboard.post("/api/auth/logout");
    } catch (error) {
