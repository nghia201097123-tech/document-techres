...
export const productService = {
  ...
  toggleActive: async (id: string): Promise<Product> => {
    const response = await api.patch<Product>(`/api/products/${id}/toggle-active`);
    return response.data;
  },
  ...
  getToppingGroupById: async (groupId: string): Promise<ToppingGroup> => {
    const response = await api.get<ToppingGroup>(`/api/products/topping-groups/${groupId}`);
    return response.data;
  },
  ...
  updateToppingGroup: async (groupId: string, data: UpdateToppingGroupDto): Promise<ToppingGroup> => {
    const response = await api.put<ToppingGroup>(`/api/products/topping-groups/${groupId}`, data);
    return response.data;
  },

  deleteToppingGroup: async (groupId: string): Promise<void> => {
    await api.delete(`/api/products/topping-groups/${groupId}`);
  },

  toggleToppingGroupActive: async (groupId: string): Promise<ToppingGroup> => {
    const response = await api.patch<ToppingGroup>(`/api/products/topping-groups/${groupId}/toggle-active`);
    return response.data;
  },
  ...
  addToppingItem: async (groupId: string, data: AddToppingItemDto): Promise<ToppingGroup> => {
    const response = await api.post<ToppingGroup>(`/api/products/topping-groups/${groupId}/items`, data);
    return response.data;
  },

  updateToppingItem: async (groupId: string, itemId: string, data: UpdateToppingItemDto): Promise<ToppingGroup> => {
    const response = await api.put<ToppingGroup>(`/api/products/topping-groups/${groupId}/items/${itemId}`, data);
    return response.data;
  },

  removeToppingItem: async (groupId: string, itemId: string): Promise<ToppingGroup> => {
    const response = await api.delete<ToppingGroup>(`/api/products/topping-groups/${groupId}/items/${itemId}`);
    return response.data;
  },

  reorderToppingItems: async (groupId: string, itemIds: string[]): Promise<ToppingGroup> => {
    const response = await api.put<ToppingGroup>(`/api/products/topping-groups/${groupId}/items/reorder`, { itemIds });
    return response.data;
  },
  ...
  getProductToppingGroups: async (productId: string): Promise<ToppingGroup[]> => {
    const response = await api.get<ToppingGroup[]>(`/api/products/${productId}/topping-groups`);
    return response.data;
  },

  assignToppingGroupsToProduct: async (productId: string, groupIds: string[]): Promise<ToppingGroup[]> => {
    const response = await api.post<ToppingGroup[]>(`/api/products/${productId}/topping-groups`, { groupIds });
    return response.data;
  },

  addToppingGroupToProduct: async (productId: string, groupId: string): Promise<ToppingGroup[]> => {
    const response = await api.post<ToppingGroup[]>(`/api/products/${productId}/topping-groups/${groupId}`);
    return response.data;
  },

  removeToppingGroupFromProduct: async (productId: string, groupId: string): Promise<ToppingGroup[]> => {
    const response = await api.delete<ToppingGroup[]>(`/api/products/${productId}/topping-groups/${groupId}`);
    return response.data;
  },
  ...
  updateNote: async (noteId: string, data: UpdateProductNoteDto): Promise<ProductNote> => {
    const response = await api.put<ProductNote>(`/api/products/notes/${noteId}`, data);
    return response.data;
  },

  deleteNote: async (noteId: string): Promise<void> => {
    await api.delete(`/api/products/notes/${noteId}`);
  },

  getProductNotes: async (productId: string): Promise<ProductNoteAssignment[]> => {
    const response = await api.get<ProductNoteAssignment[]>(`/api/products/${productId}/notes`);
    return response.data;
  },

  assignNotesToProduct: async (productId: string, noteIds: string[]): Promise<ProductNoteAssignment[]> => {
    const response = await api.post<ProductNoteAssignment[]>(`/api/products/${productId}/notes`, { noteIds });
    return response.data;
  },

  addNoteToProduct: async (productId: string, noteId: string): Promise<ProductNoteAssignment[]> => {
    const response = await api.post<ProductNoteAssignment[]>(`/api/products/${productId}/notes/${noteId}`);
    return response.data;
  },

  removeNoteFromProduct: async (productId: string, noteId: string): Promise<ProductNoteAssignment[]> => {
    const response = await api.delete<ProductNoteAssignment[]>(`/api/products/${productId}/notes/${noteId}`);
    return response.data;
  },
  ...
  getComboItems: async (comboId: string): Promise<ComboItem[]> => {
    const response = await api.get<ComboItem[]>(`/api/products/${comboId}/combo-items`);
    return response.data;
  },

  assignComboItems: async (comboId: string, items: { productId: string; quantity?: number }[]): Promise<ComboItem[]> => {
    const response = await api.post<ComboItem[]>(`/api/products/${comboId}/combo-items`, { items });
    return response.data;
  },

  addItemToCombo: async (comboId: string, productId: string, quantity?: number): Promise<ComboItem[]> => {
    const response = await api.post<ComboItem[]>(`/api/products/${comboId}/combo-items/${productId}`, { quantity });
    return response.data;
  },

  removeItemFromCombo: async (comboId: string, productId: string): Promise<ComboItem[]> => {
    const response = await api.delete<ComboItem[]>(`/api/products/${comboId}/combo-items/${productId}`);
    return response.data;
  },
